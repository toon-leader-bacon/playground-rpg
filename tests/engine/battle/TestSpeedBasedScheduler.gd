...
func test_collector_requires_player_and_enemy() -> void:
	var scheduler: SpeedBasedScheduler = _SpeedBasedScheduler.new()
	var state := BattleState.new()

	var collector: DecisionCollector = scheduler.next_collector(state)

	assert_object(collector).is_not_null()
	# Use array for lambda capture (GDScript lambda rebinding limitation)

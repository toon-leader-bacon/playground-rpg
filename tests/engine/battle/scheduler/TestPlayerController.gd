...
func test_self_targeting_move_submits_immediately() -> void:
	...
	var heal_move := _make_move("recover", 30, MoveConfig.TargetType.SELF)
...
func test_enemy_targeting_move_emits_needs_target() -> void:
	...
	var ember := _make_move("ember", 25, MoveConfig.TargetType.SINGLE_ENEMY)
...
func test_select_target_submits_correct_action() -> void:
	...
	var ember := _make_move("ember", 25, MoveConfig.TargetType.SINGLE_ENEMY)
	var scratch := _make_move("scratch", 10, MoveConfig.TargetType.SINGLE_ENEMY)
...
func _make_move(id: String, power: int, target_type: MoveConfig.TargetType) -> MoveConfig:
